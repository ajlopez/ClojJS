...
function compileTry(body, ctch, context) {
    var result = "try { " + compile(body, context) + " } catch (" + ctch.next().next().first().name() + ") { " + compile(ctch.next().next().next().first(), withLocal(context, ctch.next().next().first().name())) + " }";
    return result;
}

function compileSetBang(target, expr, context) {
    var lexpr = compile(target, context);
...
function compileFn(args, body, context) {
    var fnresult = compileFnNoRecur(args, body, context);
