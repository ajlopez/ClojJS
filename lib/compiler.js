...
function compileList(list, context) {
    ...
    if (symbols.isSymbol(first) && name) {
        const fullname = compile(first, context);
...
function compileSetBang(target, expr, context) {
    ...
    return lexpr + ' = ' + compile(expr, context);
}
...
function compileFn(args, body, context) {
    ...
    result += ") { while (true) { const $result = (" + fnresult + ")(";    
    ...
    result += "); if (!recurs.isRecur($result)) return $result; const $items = $result.items();";
...
function compileFnNoRecur(args, body, context) {
    ...
    if (restname) {
        ...
        result += " const " + restname + " = makeRest(arguments, " + names.length + ");";
    }
